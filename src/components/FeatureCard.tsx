      ...
      >
        <Card className={`card-hover ${variantClasses} ${className}`}>
          ...
          {variant !== 'numbered' && variant !== 'testimonial' && (
            <div
              className={`relative z-10 mb-4 ${variant === 'team' ? 'flex justify-center' : ''}`}
            >
              <div
                className={`w-16 h-16 bg-gradient-to-br from-primary to-blue-600 rounded-2xl flex items-center justify-center group-hover:scale-110 transition-transform duration-300`}
              >
          ...
          <div className="relative z-10 flex-1 flex flex-col">
            <h3
              className={`text-xl font-ubuntu font-bold text-neutral-text mb-3 leading-tight ${variant === 'team' ? 'text-center' : ''}`}
            >
              {title}
            </h3>
            ...
            {subtitle && (
              <p
                className={`text-sm text-primary font-semibold mb-2 ${variant === 'team' ? 'text-center' : ''}`}
              >
                {subtitle}
              </p>
            )}
            ...
            <p
              className={`text-gray-600 leading-relaxed flex-1 ${variant === 'team' ? 'text-center' : ''} ${variant === 'compact' ? 'text-sm' : 'text-base'}`}
            >
              {description}
            </p>
