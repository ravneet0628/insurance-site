  ...
  ({
    ...
    const variantClasses = React.useMemo(() => {
      ...
      const hoverClasses = enableHover
        ? 'group-hover:scale-110 transition-transform duration-300 will-change-transform'
        : '';
    ...
    const iconColorClasses = React.useMemo(() => {
      ...
      if (variant === 'soft')
        return `text-${color === 'primary' ? 'primary' : color === 'secondary' ? 'gray-600' : `${color}-600`}`;
      if (variant === 'outline')
        return `text-${color === 'primary' ? 'primary' : color === 'secondary' ? 'gray-600' : `${color}-500`}`;
      if (variant === 'none')
        return `text-${color === 'primary' ? 'primary' : color === 'secondary' ? 'gray-600' : `${color}-500`}`;
      return 'text-white';
    }, [variant, color]);
