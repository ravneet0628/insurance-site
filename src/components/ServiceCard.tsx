...
const ServiceCard: React.FC<ServiceCardProps> = React.memo(({
  ...
  const variantClasses = {
    default: 'min-h-[400px]',
    featured: 'min-h-[450px] border-2 border-primary/20',
    compact: 'min-h-[320px]'
  };
  ...
  return (
    <motion.div
      ...
      className="group"
    >
      <Card className={`h-full p-8 flex flex-col card-hover border-0 bg-white relative overflow-hidden ${variantClasses[variant]} ${className}`}>
            ...
            <div className="content-spacing">
              <ul className="space-y-2" role="list">
                {displayFeatures.map((feature, index) => (
                  <li key={`${serviceId}-feature-${index}`} className="flex items-start text-sm text-gray-600">
