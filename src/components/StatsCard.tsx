...
const StatsCard: React.FC<StatsCardProps> = React.memo(({
  ...
  return (
    <motion.div
      ...
      className="group"
    >
      <Card className={`p-6 h-full flex flex-col justify-between card-hover min-h-[160px] ${variantClasses} ${className}`}>
        {/* Background Pattern */}
        {variant !== 'minimal' && (
        ...
        <div className="flex items-start justify-between relative z-10">
          <div className="flex-1">
            ...
            <div className="flex items-baseline mb-2">
              ...
              {unit && (
                <span className={`text-lg font-medium ml-1 ${currentColorClasses.value} opacity-80`}>
                  {unit}
                </span>
              )}
            </div>
          ...
          {/* Icon */}
          {variant !== 'minimal' && (
            <div className={`w-12 h-12 rounded-xl ${currentColorClasses.bg} flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform duration-300`}>
